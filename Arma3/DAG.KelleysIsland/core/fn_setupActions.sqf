...
switch (playerSide) do
{
	case independent:
	{
		life_actions = life_actions + (player addAction["<t color = '#d02b2b'>Emergency Eject</t>",life_fnc_EmergencyEject,"",0,false,false,"",' (driver vehicle player) == player && (vehicle player) isKindOf "Air" ']);	
		...
		life_actions = life_actions + (player addAction["<t color='#ADFF2F'>Bank Teller ATM</t>",life_fnc_atmMenu,"",7,false,false,"",' (cursorTarget getVariable ["ATM_MAN", false]) && cgbankvault animationPhase "d_l_Anim" == 0 && (player distance (getMarkerPos "bank_signup")) < 21 ']);
		life_actions = life_actions + (player addAction[localize "STR_pAct_sContPlayer","","",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_sHeatPlayer",life_fnc_sheatPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_sCoolPlayer",life_fnc_scoolPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_sHydratePlayer",life_fnc_shydratePlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_sAntiViralPlayer",life_fnc_santiviralPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_sPainKillerPlayer",life_fnc_spainkillerPlayer,"",0,false,false,"",' (player getVariable ["diseaseon", 1]) > 1 ']);
		life_actions = life_actions + (player addAction[localize "STR_pAct_BandageSelf",life_fnc_bandageSelf,"",0,false,false,"",' (player getVariable ["bleedingon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_StitchSelf",life_fnc_stitchSelf,"",0,false,false,"",' (player getVariable ["woundedon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_SplintSelf",life_fnc_splintSelf,"",0,false,false,"",' (player getVariable ["fractureon", 1]) > 1 ']);

		life_actions = life_actions + (player addAction[localize "STR_pAct_EpiPenSelf",life_fnc_epipenSelf,"",0,false,false,"",' (player getVariable ["unconciouson", 1]) > 1 ']);
		life_actions = life_actions + (player addAction["<t color = '#f4a84e'>Go to the Toilet</t>",life_fnc_takeashit,"",0,false,false,"",' life_poop > 80 && (isNull objectParent player) ']);
...
' !isNull nearestObject (screenToWorld [0.5,0.5]) && count (nearestObject (screenToWorld [0.5,0.5]) getVariable ["item",[]]) > 0 && player distance nearestObject (screenToWorld [0.5,0.5]) < 5 && !life_action_inUse ']];
	...
	case civilian:
	{
		...
		life_actions = life_actions + (player addAction["<t color = '#D660D6'>Drop Player</t>",life_fnc_dropbody,"",7,false,false,"",' isdragging ']);

		life_actions = life_actions + (player addAction["<t color = '#D660D6'>Drag Player</t>",life_fnc_dragbody,"",7,false,false,"",' !life_action_inUse && vehicle player == player && (cursortarget getVariable["dead",FALSE]) && (player distance cursorTarget < 5) ']);
		life_actions = life_actions + (player addAction["<t color = '#f4a84e'>Go to the Toilet</t>",life_fnc_takeashit,"",0,false,false,"",' life_poop > 80 && (isNull objectParent player) ']);
		...
		!isNull cursorTarget && player distance cursorTarget < 7 && isPlayer cursorTarget && !(cursorTarget getVariable["robbed",FALSE]) && ( animationState cursorTarget == "Incapacitated" || cursorTarget getVariable "tied" || animationState cursorTarget == "amovpercmstpsnonwnondnon_amovpercmstpssurwnondnon" ) ']];
	...
	case west:
	{
		...
		life_actions = life_actions + (player addAction["<t color = '#ADFF2F'>Drop Player</t>",life_fnc_dropbody,"",7,false,false,"",' isdragging ']);

		life_actions = life_actions + (player addAction["<t color = '#ADFF2F'>Drag Player</t>",life_fnc_dragbody,"",7,false,false,"",' !isdragging && !life_action_inUse && vehicle player == player && (cursortarget getVariable["dead",FALSE]) && (player distance cursorTarget < 5) ']);
...
life_actions = life_actions + [player addAction["Seize Items",life_fnc_seizeObjects,cursorTarget,0,false,false,"",'(count(nearestObjects [player,["weaponholder"],3])>0)']]; 
    ...
    //Strip Search
        ...
        life_actions = life_actions + [player addAction["<t color = '#a7afec'>Strip Search</t>",life_fnc_stripDownPlayer,"",0,false,false,"",'
