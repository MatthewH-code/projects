...
if(_isVehicle) then {
		playSound3D ["dag_soimtx\sounds\caralarm.ogg", player, false, getPosASL player, 1, 1, 155];
};
while {true} do
{
...
if(!_isVehicle) then {
	...
	if(_dice < 25) then {
		...
		playSound3D ["dag_soimtx\sounds\caralarm.ogg", player, false, getPosASL player, 1, 0.8, 355];
		player setVariable ["lockpicking",false,true];
		_reason = "487";
