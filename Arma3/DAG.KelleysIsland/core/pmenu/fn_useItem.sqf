...
switch (true) do
{
	case (_item == "water"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			if (vehicle player == player) then 
			{
				...
				playSound3D ["dag_soimtx\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45];
	...
	case (_item == "battery"):
	{
		...
		{
			life_battery = 100;
			hintSilent "Your battery is now 100%.";
		};
	};	
	...
	case (_item == "feedbackstears")://Rax's Rum
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			
			playSound3D ["dag_soimtx\sounds\drink.ogg", player, false, getPosASL player, 6, 1, 45];
			titleText["You have consumed Feedbacks's Tears","PLAIN"];
	...
	case (_item == "bec"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			playSound3D ["dag_soimtx\sounds\eat.ogg", player, false, getPosASL player, 6, 1, 45];
