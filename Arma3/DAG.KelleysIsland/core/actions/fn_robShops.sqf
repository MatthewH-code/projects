...
[[1,format["911 DISPATCH: Jackson General Store has person(s) with weapons!",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
while {true} do {
		...
		if (!alive _robber) exitwith
			{
				[[1,format["911 DISPATCH: ONE ASSAILANT HAS DIED: Jackson General Store",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
				hint "STORE CLERK: Oh damn, you just got blood on my cash register.";
				sleep 300;
				_action = _shop addAction["Rob Store",life_fnc_robShops];
			};
			if (_dist >= 6) exitwith {
			[[1,format["911 DISPATCH: ONE ASSAILANT HAS FLED: Jackson General Store",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
_reason = "211";
[player,player,_reason] call life_fnc_createEvidence;

hint format["You have stolen $%1",_funds]; //Give them a nice message
_shop switchMove "";//Reseting the shop owner
[[1,format["Jackson NEWS: The Cities General Store just got robbed for $%2", _shop, [_funds] call life_fnc_numberText]],"life_fnc_broadcast",civilian,false] spawn life_fnc_MP;
_funds = 0;
