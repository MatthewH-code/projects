...
diag_log "------------------------------------------------------------------------------------------------------";
diag_log "--------------------------------- Starting DAG Life Client Init ----------------------------------";
diag_log "------------------------------------------------------------------------------------------------------";
...
[] spawn {
	...
	while {true} do {
		...
		if(_server != "DarkAresGaming" or _channel != "TaskForceRadio") then {
			
			cutText["Wrong TS Channel.","BLACK",0,true];
			waitUntil {([] call TFAR_fnc_getTeamSpeakServerName) == "DarkAresGaming" and ([] call TFAR_fnc_getTeamSpeakChannelName) == "TaskForceRadio"};
			cutText["","PLAIN"];
