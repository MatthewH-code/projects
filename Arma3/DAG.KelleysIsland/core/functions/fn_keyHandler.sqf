...
switch (_code) do
{
	...
	case 57:
	{
		...
		if(_shift && {animationState player != "AovrPercMrunSrasWrflDf"} && {isTouchingGround player} && {stance player == "STAND"} && {speed player > 2} && {!life_is_arrested} && {(velocity player) select 2 < 2.5} && {time - jumpActionTime > 1.5}) then {
			...
			[[player,false],"life_fnc_jumpFnc",nil,FALSE] call BIS_fnc_MP;
...
//code 1 key shift+1
	...
	case 6:
	{
		if(_shift) then{
			...
			[[1,format["911 DISPATCH: %1 IS IN NEED OF ASSISTANCE.",name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
			[[0,format["911 DISPATCH: %1 IS IN NEED OF ASSISTANCE", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
			[[0,format["911 DISPATCH: %1 IS IN NEED OF ASSISTANCE", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
			[[0,format["911 DISPATCH: %1 IS IN NEED OF ASSISTANCE", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		};
		};
	...
	case 19:
	{
		...
		if(_shift && playerSide == west && !isNull cursorTarget && !life_paintball && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && alive cursorTarget && cursorTarget distance player < 3 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained")) then
		{
		    [] spawn life_fnc_AntiSpam2;
	...
	{
		if(_shift) then {_handled = true;};
		if(_shift && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 1.5) then
		{
