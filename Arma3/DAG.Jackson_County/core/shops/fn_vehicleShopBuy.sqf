...
if((life_veh_shop select 0) == "med_air") then {
	...
	if(typeName _spawnPoints == typeName []) then {
		...
		{if(count(nearestObjects[(getMarkerPos _x),["Car","Motorcycle","Motorbike","Ship","Air"],5]) == 0) exitWith {_spawnPoint = _x};} foreach _spawnPoints;
	} else {
		if(count(nearestObjects[(getMarkerPos _spawnPoints),["Car","Motorcycle","Motorbike","Ship","Air"],5]) == 0) exitWith {_spawnPoint = _spawnPoints};
	};
};
...
if((life_veh_shop select 0) == "med_air") then {
	...
	_hs = nearestObjects[getMarkerPos _spawnPoint,["EMS"],50] select 0;
