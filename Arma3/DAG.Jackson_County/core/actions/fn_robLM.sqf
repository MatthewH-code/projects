...
[[1,format["911 DISPATCH: Jackson Market has an emergency alarm triggered!",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
while {true} do {
		...
		if (!alive _robber) exitwith
			{
				[[1,format["911 DISPATCH: ONE ASSAILANT HAS DIED: Jackson Market",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
				hint "STORE CLERK: Oh damn, you just got blood on my cash register.";
					sleep 1800;
				_action = _shop addAction["Rob Store",life_fnc_robShops];
			};
			if (_dist >= 6) exitwith {
			[[1,format["911 DISPATCH: ONE ASSAILANT HAS FLED: Jackson Market",_storename]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
